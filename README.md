...
### ゲームプレイ
![ゲームプレイデモ](./assets/demo-gameplay.gif)
...
このアプリはPWA対応で、以下の機能が利用できます：

- **ホーム画面に追加**: アプリのようにインストール可能
- **オフライン対応**: ネットワークがなくてもキャッシュから表示
- **高速起動**: Service Workerによる高速な起動
- **プッシュ通知**: (今後実装予定) ゲーム招待や対戦通知

### アイコンの準備

提供されたロゴ画像を以下のサイズに変換して `frontend/public/` に配置してください：
...
詳細なセットアップ手順は `frontend/PWA_SETUP.md` を参照してください。
